...
# See Notebook N7p181-186, 196-200, N8p(-4)-2, p9 and p96-109, p196-201 for math derivations
...
def dlam_2var_db(a, b, t):
    """ Derivative of Lambda, function of two rate variables (a,b) with respect to var b at t
        ...
        der: derivative, function on (a,b) domain
    """
    eps = 1e-16
...
def dlam_chr_da(a, t):
    """ Derivative of Lambda with respect to a at t
        t: time point (float)
        kR: chromatin release rate
        kND: nuclear RNA degradation (on chromatin)
        a: kR + kND, chromatin residence rate. 
        der: derivative, function on k domain 
    """
...
@jit(float64(float64, float64, float64), nopython=True, cache=True)
def dlam_nucpl_db(a, b, t):
    der = dlam_2var_db(a, b, t)
    return der

@jit(float64(float64, float64, float64), nopython=True, cache=True)
def dlam_nuc_from_chr_dkE(kR, kE, t):
    """ Derivative of Lambda, function of two rate variables (kR,kE), so kND=0, with respect to var kE at t
        t: time point (float)  
        kR: chromatin Release rate in domain [0,inf)
        kE: nuclear Export rate in domain [0,inf)
        der: derivative, function on (a,b) domain
    """
    eps = 1e-2 #smaller gives non-convergence of integration
    if np.absolute(((kR - kE) * (kR + kE))) <= eps:
        der = 4**(-1) * (t + kE * (t**2)) * np.exp(-kE * t)
    else:
        prefactor = ((kR - kE) * (kR + kE))**(-1)
        der = prefactor * (-(kR**2) * (2 * kE * prefactor - t) * np.exp(-kR * t) + \
        (2 * (kE**3) * prefactor + 2 * kE) * np.exp(-kE * t) )
    return der

@jit(float64(float64, float64, float64, float64), nopython=True, cache=True)
def dlam_nuc_from_chr_dkND(a, b, kND, t):
    """ Derivative of Lambda with respect to kD (from_nuc) at t
        t: time point (float)  
        kR: chromatin release rate in domain [0,inf)
        kND: nuclear RNA degradation (on chromatin), possibly zero
        kE: nuclear Export rate in domain [0,inf)
        a: kR + kND, chromatin residence rate.
        b: kE + kND, nucleoplasmic residence rate
        der: derivative, function on k domain
    """
    eps = 1e-16
    if np.absolute((a - b) * (a + b - kND)) <= eps:
        der = 0 #Limit approximation        
    else:
        Psi = -b * ((a - b) * (a + b - kND))**(-1) + \
              -b * (b - kND) * ((a + b - kND)**2 * (a - b))**(-1)
        Phi = a * ((a - b) * (a + b - kND))**(-1) + \
              a * (a - kND) * ((a + b - kND)**2 * (a - b))**(-1)
        der = Psi * np.exp(-a * t) + \
              Phi * np.exp(-b * t)
    return der
...
@jit(float64(float64, float64, float64, float64), nopython=True, cache=True)
def dlam_cyto_from_chr_dkD(a, b, kD, t):
    """ Derivative of Lambda with respect to kD (from_nuc) at t
        t: time point (float)  
        kR: chromatin release rate in domain [0,inf)
        kND: nuclear RNA degradation (on chromatin), possibly zero
    ...
    if np.absolute(a - b) <= eps or np.absolute(kD - a) <= eps or np.absolute(kD - b) <= eps:
        ...
        Psi = -a * b * ((kD - a)**2 * (a - b))**(-1)
        Phi = a * b * ((kD - b)**2 * (a - b))**(-1)
        Omega = a * b * ((kD - a)**2 * (kD - b))**(-1) + \
                a * b * ((kD - b)**2 * (kD - a))**(-1) + \
                a * b * t * ((kD - b) * (kD - a))**(-1)
        der = Psi * np.exp(-a * t) + \
              Phi * np.exp(-b * t) + \
...
#Nuclear degradation model (v1: from nuc)
@jit(float64(float64, float64, float64, float64), nopython=True, cache=True)
...
#Nuclear degradation model (v2: from chr)
@jit(float64(float64, float64, float64, float64, float64), nopython=True, cache=True)
def dlam_total_from_chr_dkND(a, b, kD, kND, t):
    """ Derivative of Lambda with respect to kD (from_nuc) at t
        t: time point (float)  
        kR: chromatin release rate in domain [0,inf)
        kND: nuclear RNA degradation (on chromatin), possibly zero
    ...
    """
    eps = 1e-16
    if (np.absolute(a - b) <= eps) or (np.absolute(kD - a) <= eps) or \
       (np.absolute(kD - b) <= eps) or (np.absolute(kD * (a + b - kND) + (a - kND) * (b - kND)) <= eps):
        der = 0 #limit approximation       
    else:
        pf1 = b * kD * (kD * (a + b - kND) + (a - kND) * (b - kND))**(-1) #prefactor1
        pf2 = (-2 * kND + a + b + kD) * (kD * (a + b - kND) + (a - kND) * (b - kND))**(-1) #prefactor2
        
        Psi = (pf2 * (b - kND) * (kD - kND) + 2 * kND - b - kD) * ((a - b) * (kD - a))**(-1)
        Phi = -a * (pf2 * (a - kND) * (kD - kND) + 2 * kND - a - kD) * (b * (a - b) * (kD - b))**(-1)
        Omega = -a * (pf2 * (a - kND) * (b - kND) + 2 * kND - a - b) * (kD * (kD - a) * (kD - b))**(-1)
        der = pf1 * (Psi * np.exp(-a * t) + \
                     Phi * np.exp(-b * t) + \
                     Omega * np.exp(-kD * t))     
    return der
...
@jit(float64(float64, float64, float64), nopython=True, cache=True)
def det_jac_nuc_from_chr_nucres(kR, kE, t):
    det = np.absolute(dlam_chr_da(kR, t) * dlam_nuc_from_chr_dkE(kR, kE, t))
    return det

@jit(float64(float64, float64, float64, float64), nopython=True, cache=True)
def det_jac_nuc_from_chr_nucdeg(a, b, kND, t):
    det = np.absolute(dlam_chr_da(a, t) * dlam_nucpl_db(a, b, t) * dlam_nuc_from_chr_dkND(a, b, kND, t))
    return det
...
@jit(float64(float64, float64, float64, float64), nopython=True, cache=True)
def det_jac_cyto_from_chr(a, b, kD, t):
    det = np.absolute(dlam_chr_da(a, t) * dlam_nucpl_db(a, b, t) * dlam_cyto_from_chr_dkD(a, b, kD, t))
    return det
...
@jit(float64(float64, float64, float64, float64, float64), nopython=True, cache=True)
def det_jac_total_from_chr(a, b, kD, kND, t):
    det = np.absolute(dlam_chr_da(a, t) * dlam_nucpl_db(a, b, t) * dlam_cyto_from_chr_dkD(a, b, kD, t) * \
                      dlam_total_from_chr_dkND(a, b, kD, kND, t))
    return det
