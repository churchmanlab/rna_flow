...
# use: ./top100genes.R *.tsv *ptc.genes.txt timepoint
...
# set new file name
new_file_name1 <- paste(sample, "_bottom500genes_turnover.bed", sep="")
new_file_name2 <- paste(sample, "_bottom500.MAPs.txt", sep="")
new_file_name3 <- paste(sample, "_bottom500_below.MAPs.txt", sep="")
new_file_name4 <- paste(sample, "_default_high_background.MAPs.txt", sep="")
...
# Here enrich for genes with some but not much labeling (20% MAPs and more)
out <- background_filtered %>% filter(background_filtered[,column]>0.2)
out <- out %>% arrange(out[,column])
out <- out[1:500,]

# Retain only gene and timepoint MAP values
out <- out[,c(1,2,column)]

# ADDED - genes that fall below this cutoff too
out2 <- background_filtered %>% filter(background_filtered[,column]<=0.2)
out2 <- out2 %>% arrange(out2[,column])
out2 <- out2[,c(1,2,column)]

# ADDED - genes with high background that could not be considered
out3 <- background_filtered %>% filter(background_filtered[,10] >= 0.01)
out3 <- out3 %>% arrange(out3[,column])
out3 <- out3[,c(1,2,column)]
...
write.table(out2, new_file_name3, quote=FALSE, row.names=FALSE, col.names=FALSE, sep="\t")
write.table(out3, new_file_name4, quote=FALSE, row.names=FALSE, col.names=FALSE, sep="\t")
