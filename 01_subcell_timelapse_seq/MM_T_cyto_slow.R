...
MapMethod <- 'NA' 
reads <- 'bottom500genes_turnover' # 'MTall' All MTnorRNA
...
for (i in c(1:NumSamps)) {
  # get reads
  assign(paste0('mDT',i), data.table(read.table(paste0(path, 'STAR_2023/', samples[i],'/',samples[i],'_MM_temp_turnover_slow/',samples[i],'_frag_MMfrequency_All.txt'), header=TRUE, sep='\t', quote='',stringsAsFactors = FALSE)))
...
for (i in c(1:NumSamps)){
  ...
  write.table(JointFreq, file=paste0(path, 'STAR_2023/', samples[i],'/',samples[i],'_MM_temp_turnover_slow/',samples[i],'_',reads,'_TcountANDTCconv.txt'), sep=("\t"), quote=FALSE, col.names=NA)
}
