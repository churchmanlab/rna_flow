...
# old applyMW (Marco J asked me to change it b/c of issue with mann-whitney and new SciPy version 1.7.1)

# "PSA: if you are using the screenprocessing pipeline to analyze crispr screens and using scipy v1 or higher, your mann-whitney p-values are probably inflated by a factor of 2. Changing the "applyMW" code in process_experiments.py as follows should fix it but I have not confirmed this 100% so use at your own risk:"
...
# new applyMW
def applyMW(group, negativeTable):
    if (int(sp.__version__.split('.')[1]) < 17) and (int(sp.__version__.split('.')[0]) == 0): #implementation of the "alternative flag":
    ...
    elif (int(sp.__version__.split('.')[1]) >= 7) and (int(sp.__version__.split('.')[0]) == 1): #implementation of the "alternative flag":
        return group.apply(lambda column: stats.mannwhitneyu(column.dropna().values, negativeTable[column.name].dropna().values, alternative = 'two-sided', method='asymptotic')[1] if len(column.dropna()) > 0 else np.nan)
    else:
        return group.apply(lambda column: stats.mannwhitneyu(column.dropna().values, negativeTable[column.name].dropna().values, alternative = 'two-sided')[1] if len(column.dropna()) > 0 else np.nan)
