...
# Source: `Timescale_Bayes_20221117_rev1p1.ipynb`  
# For RNA flow project: 
# Bayesian inference to get timescale posteriors based on Grand-Slam new to total ratio posteriors for individual genes, now including **nuclear degradation**. 

# This script is used for batch scripts used to run for all genes together with `Timescale_Bayes_20230128.sh`.   
...
def main():
    ...
    eps_underflow = 1e-250 #minimally 1e-300. see sys #identical to 1/underflow correction value in post_joint_from_nucdeg.
    ...
    outpath = os.path.join('/n','groups','churchman','ri23','bseq','Bayes20230128')

    # Add a logger specific to the project and processing stage
    logger = logging.getLogger('Bayes_rate')
    log_file = os.path.join(outpath,'LogErr', 'scTLseq_bayes_20230128_%s_batch%s_py.log' % 
    ...
    def post_joint_from_nucdeg(kR, kE, kD, kND, TC, NTR, underflow, pr_not_post=False):
        ...
        '''
        if not underflow:
            prob = pr_chr((kR + kND), TC['chr'], NTR['chr']) \
        ...
        else:#constant factor 1e250 to overcome the computer float precision of 1e-304
            prob = 1e250 * pr_chr((kR + kND), TC['chr'], NTR['chr']) \
                   * pr_nuc_from_chr((kR + kND), (kE + kND), kND, TC['nuc'], NTR['nuc']) \
                   * pr_cyto_from_chr((kR + kND), (kE + kND), kD, TC['cyto'], NTR['cyto']) \
                   * pr_total_from_chr((kR + kND), (kE + kND), kD, kND, TC['tot'], NTR['tot'], pr_not_post)
        return prob
    ...
    def get_estimates_from_post(post, TC, NTR, N_var, ci_k_prev=None, prevp=None):
        ...
        if N_var != 4:
            ...
            if (max_post > eps_post) and (norm_const > eps_underflow):
            ...
            if (max_post > eps_post) and (llh_nucres > eps_underflow):
                ...
                post_grid, norm_const = get_post_from_nucdeg(post, TC, NTR, 
    ...
    filename = 'Bayes_Rates_20230128_'+org_map[o]+'_batch'+str(args.start_id)+'.tsv'
            ...
            for fr in fracs:           
                if r+fr+'top1000' in GS[o].keys():
                    for estimate_type in OUT_TYPES[:2]:
                        ...
                        try:
                            TC_TYPES_gene = fit.get_tc_types_for_gene_jit(ensid, r, fr, GS[o].keys(), TB)
                            ...
                            for i in TC_TYPES_gene:
                                tc = TC_from_jit[i]                            
                                gene_idx = GS[o][r+fr+tc][GS[o][r+fr+tc]['Gene']==ensid].index[0]
                                NTR = NTR + np.asarray(GS[o][r+fr+tc].loc[gene_idx, gs_times], dtype='float64')
